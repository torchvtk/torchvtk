...
def main():
    ...
    # tfms = NoiseDictTransform(std_deviation=0.01, device="cpu", apply_on=["vol"])
    # tmp = tfms(file)
    ...
    # file = load_file()
    #
    # tfms = NoiseDictTransform(device="cuda", apply_on=["vol"], std_deviation=0.01)
    # tmp = tfms(file)
    # view_batch(tmp["vol"].squeeze(), width=512, height=512)
    # del tfms, file
    #
    ...
    # Cropping CPU.
    file = load_file()
    tfms = CroppingTransform(device="cpu", apply_on=["vol", "mask"], dtype=torch.float32, size=(64, 256,256))
    tmp = tfms(file)
